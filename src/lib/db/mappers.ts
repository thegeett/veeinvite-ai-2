...
export function rowToCouple(row: JsonRow): CoupleData {
  return {
    ...
    cultures: asArray<CultureSelection>(row.cultures),
    vibe_tags: asArray<string>(row.vibe_tags),
    expressive_palette: asJson<ExpressivePalette | null>(row.expressive_palette, null),
