...
--
--   invitation-sites   PRIVATE. Served by /w/[slug] route handler, which reads
--                      the HTML via the service-role client, substitutes photo
--                      placeholder markers with freshly-signed 1-hour URLs,
--                      and returns to the browser. Access is gated on
--                      couples.is_published.
--
--   preview-sites      PRIVATE. Served by /preview/[token] route handler,
--                      which validates the token against preview_tokens
--                      (expiry + revocation) and follows the same signed-URL
--                      substitution flow as invitation-sites.
--
--   couple-photos      PRIVATE. Couple photo uploads. NEVER publicly
--                      accessible. Accessed only via signed URLs (≤1 hour
--                      expiry) generated by the /w/[slug] and /preview/[token]
--                      route handlers at serve time. Protects photo reputation
--                      — an expired or leaked URL stops working within an
--                      hour, preventing scraping and redistribution.
--
-- Upload paths:
--   invitation-sites/{slug}.html
--   preview-sites/{token}.html
--   couple-photos/{couple_id}/{uuid}.{ext}
--
...
-- against the couple-photos bucket.
