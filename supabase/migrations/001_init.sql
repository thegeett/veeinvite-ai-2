...
-- across all Supabase versions. Required buckets:
--   - sites      (public read, service-role write)   — generated HTML
--   - photos     (public read, service-role write)   — couple photo uploads
--   - previews   (public read, service-role write)   — shareable preview HTML
--
-- After the buckets exist, set their policies so that authenticated couple
-- owners can upload via signed URLs issued by Stream C's API routes.
